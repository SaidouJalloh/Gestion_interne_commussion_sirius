...
type ActiviteContrat = {
    id: string;
    date_expiration?: string;
    commission?: number | string | null;
    clients?: { nom?: string | null; prenom?: string | null; type_client?: string | null } | null;
    compagnies?: { nom?: string | null } | null;
};

type ActivitePaiement = {
    id: string;
    type_paiement?: string | null;
    montant?: number | string | null;
    contrats?: { clients?: { nom?: string | null; prenom?: string | null } | null } | null;
};

type Activites = {
    derniers_contrats: ActiviteContrat[];
    derniers_paiements: ActivitePaiement[];
    contrats_expirants: ActiviteContrat[];
};

export const ActivitesRecentes = ({ activites }: { activites: Activites }) => {
                    ...
                    {activites.derniers_contrats.map((contrat) => (
                        ...
                        >
                            ...
                            <p className="text-xs text-primary-600 dark:text-primary-400 font-semibold mt-1">
                                {formatCurrency(contrat.commission ?? 0)}
                            </p>
                        </div>
                    ))}
                </div>
