...
export default function DefaultLayout({ children }: { children: ReactNode }) {
    ...
    // Menu simple pour utilisateurs par défaut
    const menuItems = [
        {
            name: 'Clients',
            path: '/clients',
    ...
    return (
        <div className="flex h-screen bg-gray-50 dark:bg-gray-900 transition-colors">
            {/* Sidebar simple */}
            <aside className={`${sidebarOpen ? 'w-64' : 'w-20'} bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700 text-gray-900 dark:text-white transition-all duration-300 ease-in-out shadow-sm relative`}>
                ...
                <nav className="mt-6 px-3 pb-32 overflow-y-auto" style={{ maxHeight: 'calc(100vh - 200px)' }}>
                    {menuItems.map((item) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-3 px-3 py-3 mb-2 rounded-lg transition-all duration-200 ${
                                isActive(item.path)
                ...
                <div className="absolute bottom-0 left-0 w-full p-4 border-t border-gray-200 dark:border-gray-700">
                    <button
                        ...
                        className={`w-full flex items-center gap-3 px-3 py-2 hover:bg-gray-50 dark:hover:bg-gray-700 rounded-lg transition-colors ${!sidebarOpen && 'justify-center'}`}
                    >
                        <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div className="flex items-center gap-3 px-4 py-2 bg-gray-50 dark:bg-gray-700 rounded-lg">
                            {profile?.avatar_url ? (
                                ...
                                <div className="w-10 h-10 rounded-full bg-gray-400 flex items-center justify-center text-white font-bold text-lg">
                                    {profile?.prenom?.[0]?.toUpperCase() || user?.email?.[0]?.toUpperCase() || 'U'}
                                </div>
                            )}
